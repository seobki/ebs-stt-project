...
class STTProcessor:
    ...
    def __init__(self, model_size="", device="cuda", compute_type="float32"):
        if not model_size:
            ...
            print(f"❌ 지원하지 않는 모델 크기입니다: {model_size}. 'small'으로 설정합니다.")
            model_size = "small"

        self.model_size = model_size
        print(f"Whisper 모델 로드 중... 크기: {model_size}, 디바이스: {device}, 타입: {compute_type}")
        
        self.model = WhisperModel(model_size, device=device, compute_type=compute_type)
    
    def stt_whisper(self, output_file_wav):
    
        print(f"STT 시작 : {output_file_wav}")
        start_time = datetime.now()
        
        try:
            segments, info = self.model.transcribe(output_file_wav, beam_size=3)
            print(f"언어 감지 : {info.language}, (확신도: {info.language_probability:.2f})")
            ...
            print(f"STT 종료 : {elapsed_str} (시작시간 : {start_time.strftime('%H:%M:%S')}, 종료시간 : {end_time.strftime('%H:%M:%S')})")
...
def save_to_json(stt_results, output_file_json):
    ...
    try:
        ...
        print(f"STT 결과 JSON 파일 저장 성공! : {output_file_json}")
...
#         print(f"STT 종료 : {elapsed_str} (시작시간 : {start_time.strftime('%H:%M:%S')}, 종료시간 : {datetime.now().strftime('%H:%M:%S')})")
